...
export default function Portfolio() {
  ...
  const levelColor = (level) => {
    if (level === "Advanced") return isDarkMode ? "text-emerald-400" : "text-emerald-600";
    if (level === "Intermediate") return isDarkMode ? "text-blue-400" : "text-blue-600";
    return isDarkMode ? "text-neutral-500" : "text-neutral-400";
  };
    ...
    >
      ...
      <div className="max-w-md mx-auto px-6 pt-16 space-y-10 relative z-10">
        ...
        <div className="space-y-4">
          <h2 className={`text-xl font-bold transition-colors ${isDarkMode ? 'text-white' : 'text-neutral-900'}`}>Skills</h2>
          <div className="space-y-3">
            {skills.map((skill, index) => (
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={`w-11 h-11 rounded-full flex items-center justify-center shadow-inner border transition-colors
                    ${isDarkMode ? 'bg-neutral-900 border-neutral-800 text-neutral-300' : 'bg-neutral-100 border-neutral-200 text-neutral-600'}`}
                  >
                    {skill.icon}
                  </div>
                  <div>
                    ...
                    <p className={`text-xs mt-0.5 font-medium ${levelColor(skill.level)}`}>
                      {skill.level}
                    </p>
                  </div>
                </div>
        ...
        >
          ...
          >
            <div className={`w-3.5 h-3.5 rounded-[2.5px] transform rotate-12 flex items-center justify-center transition-colors
              ${isDarkMode ? 'bg-neutral-400 group-hover:bg-white text-[#0a0a0a]' : 'bg-neutral-500 group-hover:bg-neutral-900 text-white'}`}
            >
